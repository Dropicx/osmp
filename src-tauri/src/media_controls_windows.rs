...
use tokio::sync::mpsc;
use windows::{
    core::*, Foundation::*, Media::MediaPlaybackStatus, Media::SystemMediaTransportControls,
};
...
impl WindowsControls {
    pub fn new(
        ...
        Ok(WindowsControls { smtc, event_sender })
    }
}
...
impl MediaControlsPlatform for WindowsControls {
        ...
        if let Some(ref controls) = *smtc_guard {
            unsafe {
                ...
                let updater = controls
                    .GetDisplayUpdater()
                    .map_err(|e| format!("Windows error: {:?}", e))?;

                // Set media type to Music
                updater
                    .SetType(1)
                    .map_err(|e| format!("Windows error: {:?}", e))?; // MediaPlaybackType::Music = 1
    ...
    fn update_playback_state(&self, state: PlaybackState) -> Result<(), String> {
        ...
        if let Some(ref controls) = *smtc_guard {
            unsafe {
                ...
                controls
                    .SetPlaybackStatus(playback_status)
                    .map_err(|e| format!("Windows error: {:?}", e))?;
            }
        }
        Ok(())
    ...
    fn set_available_actions(
        ...
        if let Some(ref controls) = *smtc_guard {
            unsafe {
                controls
                    .SetIsNextEnabled(can_go_next)
                    .map_err(|e| format!("Windows error: {:?}", e))?;
                controls
                    .SetIsPreviousEnabled(can_go_previous)
                    .map_err(|e| format!("Windows error: {:?}", e))?;
            }
        }
        Ok(())
