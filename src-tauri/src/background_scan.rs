            ...
            match db.lock() {
                Ok(db_lock) => {
                    let startup = db_lock
                        ...
                        .map(|v| v == "true")
                        .unwrap_or(true);
                    let periodic = db_lock
                        .get_setting("periodic_scan_enabled")
                        .ok()
                        .flatten()
                        .map(|v| v == "true")
                        .unwrap_or(true);
                    let interval = db_lock
                ...
                Err(e) => {
                    error!(
                        "Failed to lock database for background scan settings: {}",
                        e
                    );
                    return;
                }
...
async fn run_background_scan(
    ...
    let enabled_folders: Vec<String> = match db.lock() {
        Ok(db_lock) => match db_lock.get_scan_folders() {
            Ok(folders) => folders
                .into_iter()
                .filter(|f| f.enabled)
                .map(|f| f.path)
    ...
    let result = tokio::task::spawn_blocking(move || {
        let scanner =
            ScannerWithProgress::new_quiet(db_clone, app_handle_clone, scan_cancelled_clone);
